...
class CreateUserActivity : AppCompatActivity() {
    ...
    private fun loadPersons() {
        val url = "http://192.168.1.83:9191/api/Person" // Ajusta la URL según sea necesario
    ...
    private fun checkIfPasswordExists(password: String, callback: (Boolean) -> Unit) {
        val url = "http://192.168.1.83:9191/api/User/checkPassword" // Ajusta según tu API
        val queue = Volley.newRequestQueue(this)
    ...
    private fun sendCreateUserRequest(personId: Int, userName: String, password: String) {
        val url = "http://192.168.1.83:9191/api/User"
        val queue = Volley.newRequestQueue(this)
